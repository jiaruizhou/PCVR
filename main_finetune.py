...
from engine_finetune import train_one_epoch, evaluate, predict,evaluate_single

from util.custom_datasets import finetune_Dataset
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_dir_finetune2', help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir_finetune2', help='path where to tensorboard log')
    ...
    # my settings
    parser.add_argument('--weight1', default=0.333, type=float)
    parser.add_argument('--weight2', default=0.333, type=float)
    parser.add_argument('--weight3', default=0.333, type=float)
    ...
    parser.add_argument('--use_imagenet', default=False, type=bool)
...
def main(args):
    ...
    args.output_dir += "/{}/{}".format(args.data, args.tax_rank)
    ...
    if not args.eval:
        ...
        dataset = finetune_Dataset(args, files=args.data_path, kmer=args.kmer, phase="train")
    ...
    if args.eval:
        dataset_val = finetune_Dataset(args, files=args.data_path, kmer=args.kmer, phase="test")
    
    if args.use_imagenet:

        model = models_vit.RetrievalNet()
    else:
        if args.only==0:
    ...
    if args.finetune and not args.eval:
        ...
        if args.unknown_loss :# and args.data=="final"
            for k in ['head.weight', 'head.bias', 'head2.weight', 'head2.bias', 'head3.weight', 'head3.bias','head4.weight', 'head4.bias']:
            ...
            for k in ['head.weight', 'head.bias', 'head2.weight', 'head2.bias', 'head3.weight', 'head3.bias']:
                if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
                    print(f"Removing key {k} from pretrained checkpoint")
                    del checkpoint_model[k]
        ...
        if args.global_pool:
            if "flat" in args.tax_rank:
                assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias', 'head2.weight', 'head2.bias', 'head3.weight', 'head3.bias'}
            elif args.unknown_loss :# and args.data=="final"
                assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias', 'head2.weight', 'head2.bias', 'fc_norm2.weight', 'fc_norm2.bias', 'head3.weight', 'head3.bias','head4.weight', 'head4.bias', 'fc_norm3.weight', 'fc_norm3.bias'}
            elif args.only!=0:
                ...
                assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias', 'head2.weight', 'head2.bias', 'fc_norm2.weight', 'fc_norm2.bias', 'head3.weight', 'head3.bias', 'fc_norm3.weight', 'fc_norm3.bias'}

        else:
            assert set(msg.missing_keys) == {'head.weight', 'head.bias','head2.weight', 'head2.bias', 'head3.weight', 'head3.bias'}
    ...
    if True:  # args.distributed:
        ...
        if not args.eval and not args.pred:
    ...
    if args.eval:
        if args.only==0:
            ...
            test_stats = evaluate_single(args, data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images for superkingdom: {test_stats['acc1_0']:.1f}%")
            print(f"Accuracy of the network on the {len(dataset_val)} test images for phylum: {test_stats['acc1_1']:.1f}%")
            print(f"Accuracy of the network on the {len(dataset_val)} test images for genus: {test_stats['acc1_2']:.1f}%")
            exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, criterion, data_loader_train, optimizer, device, epoch, loss_scaler, args.clip_grad, mixup_fn, log_writer=log_writer, model_without_ddp=model_without_ddp,  args=args)
